//https://launchschool.com/lessons/08996120/assignments/b425bdee
...
function mailCount(emailData) {
  let emailCount = emailData.split('##||##').length;
  let regex = /[\d]{2}-[\d]{2}-[\d]{4}/g;
  let dates = emailData.match(regex);
  let sortedDates = dates.sort((a, b) => new Date(a) - new Date(b));
  let beginningDate = new Date(sortedDates[0]);
  let endingDate = new Date(sortedDates[dates.length - 1]);

  console.log(`Count of Email: ${emailCount}`);
  console.log(`Date Range: ${beginningDate.toDateString()} - ${endingDate.toDateString()}`)
}
