// https://launchschool.com/lessons/08996120/assignments/74d8d8ca
...
function findPositiveWords(string) {
  return string.match(positiveRegex);

}

function findNegativeWords(string) {
  return string.match(negativeRegex);
}
...
function sentiment(text) {
  let posWords = findPositiveWords(text).map(toLowerCaseWord);
  console.log(posWords);
  let negWords = findNegativeWords(text).map(toLowerCaseWord);
  console.log(negWords);
  let numPosWords = posWords.length;
  let numNegWords = negWords.length;

  console.log(`There are ${numPosWords} positive words in the text.`);
  console.log(`Positive sentiments: ${posWords.join(', ')}`);
  console.log(`There are ${numNegWords} negative words in the text.`);
  ...
  console.log(`The sentiment of the text is ${sentimentAnalysis}.`);
